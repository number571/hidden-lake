...
func TestHandler(t *testing.T) {
	...
	server := testInitInternalServiceHTTP(ctx, httpLogger, newTsHLKClient(0), testutils.TgAddrs[23])
	...
	if _, err := client.ExecCommand(context.Background(), "abc", ""); err != nil {
		t.Fatal(err)
	}
}
...
func testInitInternalServiceHTTP(
	...
	mux.HandleFunc(
		...
		HandleCommandExecAPI(pCtx, pLogger, pHlkClient),
	) // GET
