...
func (p *sApp) initStorage() error {
	...
	if err := os.MkdirAll(stgPrivatePath, 0700); err != nil { //nolint:gosec
		return err
	}
	stgSharingPath := filepath.Join(p.fPathTo, hls_filesharer_settings.CPathSharingSTG)
	if err := os.MkdirAll(stgSharingPath, 0700); err != nil { //nolint:gosec
		return err
	}
	stgSharingPublicPath := filepath.Join(p.fPathTo, hls_filesharer_settings.CPathSharingPublicSTG)
	if err := os.MkdirAll(stgSharingPublicPath, 0700); err != nil { //nolint:gosec
		return err
	}
	return nil
}
