...
func TestInitApp(t *testing.T) {
	...
	if err := os.WriteFile(tcPathConfig+"hla-meshtastic.yml", []byte(tcDataConfig), 0600); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := InitApp([]string{"--path", tcPathConfig + "/failed/1"}, tgFlags); err == nil {
		t.Fatal("success init app with invalid config (1)")
	}
...
func testDeleteFiles(path string) {
	...
	_ = os.RemoveAll(path + settings.CPathPy)
	_ = os.RemoveAll(path + settings.CPathTxt)
	_ = os.RemoveAll(path + settings.CPathVenv)
}
