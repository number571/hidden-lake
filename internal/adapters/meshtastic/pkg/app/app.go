...
import (
	...
	hla_meshtastic_config "github.com/number571/hidden-lake/internal/adapters/meshtastic/pkg/config"
	...
	hla_meshtastic "github.com/number571/hidden-lake/pkg/network/adapters/meshtastic"
)
...
type sApp struct {
	...
	fExtAdapter hla_meshtastic.IMeshtasticAdapter
}
		...
		fIntAdapter: hla_http.NewHTTPAdapter(
			hla_http.NewSettings(&hla_http.SSettings{
				...
				FServeSettings: &hla_http.SServeSettings{
					...
					FSubscribeID:   fmt.Sprintf("%s-%s", hla_meshtastic_settings.CAppShortName, random.NewRandom().GetString(16)),
...
func (p *sApp) enable(pCtx context.Context) state.IStateF {
	return func() error {
		...
		p.fStdfLogger.PushInfo(fmt.Sprintf(
			"%s is started; %s",
			hla_meshtastic_settings.GetAppShortNameFMT(),
			encoding.SerializeJSON(hla_meshtastic_config.GetConfigSettings(p.fConfig)),
		))
		return nil
	}
}
