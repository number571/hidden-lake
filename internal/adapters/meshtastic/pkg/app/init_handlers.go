...
import (
	...
	"github.com/number571/hidden-lake/internal/adapters/meshtastic/internal/handler"
)
...
func (p *sApp) initHandlers(pCtx context.Context) {
	p.fIntAdapter.WithHandlers(map[string]http.HandlerFunc{
		...
		hla_settings.CHandleConfigSettingsPath: handler.HandleConfigSettingsAPI(p.fConfig, p.fHTTPLogger),
		hla_settings.CHandleConfigConnectsPath: handler.HandleConfigConnectsAPI(pCtx, p.fConfig, p.fHTTPLogger),
