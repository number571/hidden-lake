...
func TestHandleConfigConnectsAPI(t *testing.T) {
	...
	handler := HandleConfigConnectsAPI(ctx, &tsConfig{}, log)
	...
	if err := configConnectsRequestAddConnection(handler, http.StatusNoContent); err != nil {
		t.Fatal(err)
	}
	if err := configConnectsRequestDelConnection(handler, http.StatusNoContent); err != nil {
		t.Fatal(err)
	}
}
...
func configConnectsRequestAddConnection(handler http.HandlerFunc, code int) error {
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader("meshtastic://abc"))
...
func configConnectsRequestDelConnection(handler http.HandlerFunc, code int) error {
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodDelete, "/", strings.NewReader("meshtastic://abc"))
...
func (p *tsConfig) GetConnection() config.IConnection { return &tsConnection{} }

type tsConnection struct{}

func (p *tsConnection) GetDevPath() string { return "" }
func (p *tsConnection) GetChannel() uint8  { return 0 }
