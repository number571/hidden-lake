...
func configConnectsRequestAddConnection(handler http.HandlerFunc, code int) error {
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader("https://abc"))
...
func configConnectsRequestDelConnection(handler http.HandlerFunc, code int) error {
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodDelete, "/", strings.NewReader("https://abc"))
...
func (p *tsConfig) GetAuthMapper() map[string]string {
	return map[string]string{"username1": "password1"}
}

type tsAddress struct{}

func (p *tsAddress) GetCertTLS() string  { return "localhost" }
